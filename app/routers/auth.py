...
def do_login(
    ...
    if role == "manager":
        if not (MANAGER_PHONE and MANAGER_NID and MANAGER_PASSWORD):
            return templates.TemplateResponse("login.html", {"request": request, "error": "مدیر تنظیم نشده (ENV خالی است)."})
        if p != normalize(MANAGER_PHONE) or n != normalize(MANAGER_NID) or password != MANAGER_PASSWORD:
            return templates.TemplateResponse("login.html", {"request": request, "error": "مشخصات مدیر اشتباه است."})
        ...
        conn.close()
        if not row or not verify_pw(password, row["password"]):
            return templates.TemplateResponse("login.html", {"request": request, "error": "کاربر یافت نشد یا رمز اشتباه است. ثبت‌نام کنید."})
...
def signup(
    ...
    if password1 != password2:
        return templates.TemplateResponse("login.html", {"request": request, "error": "رمز عبور و تکرار یکسان نیست."})
    p = normalize(phone)
    n = normalize(nid)
