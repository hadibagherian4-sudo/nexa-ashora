...
def start(request: Request):
    ...
    # صفحه شروع: پس زمینه هم‌رنگ نوار بالا + لوگو در topbar حذف
    return templates.TemplateResponse(
        "landing.html",
        {
            "request": request,
            "body_class": "is-landing",
            "hide_top_logo": True,
            "subtitle": "",
            "title": "شروع | نکسا",
        },
...
def home(request: Request):
    ...
    if role == "user":
        ...
        conn.close()
        return templates.TemplateResponse("home_user.html", {"request": request, "s": s, "published": published, "mysubs": mysubs, "subtitle": "👤 پنل کاربر"})
    ...
    if role == "manager":
        ...
        conn.close()
        return templates.TemplateResponse("home_manager.html", {"request": request, "s": s, "queue": queue, "pend_posts": pend_posts, "subtitle": "🛡️ پنل مدیر"})
    ...
    """, (s["phone"],)).fetchall()
    conn.close()
    return templates.TemplateResponse("home_referee.html", {"request": request, "s": s, "tasks": tasks, "subtitle": "🧑‍⚖️ پنل داور"})
